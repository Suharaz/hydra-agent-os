// OpenRouter chat-completions client for the cold lane. One request per call; the agent loop
// owns tool iteration. Records every call in `llm_calls` with the model the router actually used.
...
const REFERER = "https://github.com/Suharaz/hydra-agent-os";
...
export function buildBody(req: ChatRequest): Record<string, unknown> {
  const body: Record<string, unknown> = {
    ...
    provider: { require_parameters: true, allow_fallbacks: true, ...req.provider },
    usage: { include: true },
  };
...
async function post(req: ChatRequest, deps: ChatDeps, body: string): Promise<Response> {
  ...
  const url = `${deps.baseUrl ?? OPENROUTER_BASE_URL}/chat/completions`;
  ...
  try {
    ...
    throw new LlmError("http", 0, `openrouter fetch failed: ${err instanceof Error ? err.message : String(err)}`);
  }
}
...
export async function chat(req: ChatRequest, deps: ChatDeps): Promise<ChatResult> {
  ...
  if (res.status === 429) {
    ...
    log.warn("openrouter 429; retrying once", { agent: req.agent, model: req.model, retryAfterMs: ms });
  ...
  if (res.status === 402) {
    await res.body?.cancel();
    log.error("openrouter 402: out of credits", { agent: req.agent, model: req.model });
  ...
  if (res.status === 429) {
    await res.body?.cancel();
    throw new LlmError("rate", 429, "openrouter: rate limited after retry");
  }
  if (!res.ok) {
    const text = await res.text().catch(() => "");
    throw new LlmError("http", res.status, `openrouter ${res.status}: ${text.slice(0, 300)}`);
  }
  ...
  try {
    ...
    throw new LlmError("parse", res.status, `openrouter: invalid JSON body (${err instanceof Error ? err.message : String(err)})`);
  }
  const latencyMs = (nowNs() - t0) / 1e6;
  if (json.error !== undefined) {
    throw new LlmError("http", json.error.code ?? res.status, `openrouter error: ${json.error.message ?? "unknown"}`);
  }
  const choice = json.choices?.[0];
  if (choice === undefined || choice.message === undefined) {
    throw new LlmError("parse", res.status, "openrouter: response has no choices");
  }
  ...
  if (rawCost === undefined || rawCost === null) {
    log.warn("openrouter response missing usage.cost; llm budget tracking may undercount", { agent: req.agent, model: modelUsed });
  }
  const usage = {
